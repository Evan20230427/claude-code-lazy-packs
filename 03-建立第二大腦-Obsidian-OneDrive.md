---
title: 'Claude Code 懶人包 #03（OneDrive 版）：建立第二大腦（Obsidian）'
date: '2026-05-20'
type: 懶人包
version: v0.1
status: OneDrive 版
...
video: EP06
---
# Claude Code 懶人包 #03（OneDrive 版）：建立第二大腦（Obsidian）
...
- [ ] 已有 Microsoft 帳號（用於 OneDrive 同步）
- [ ] 電腦有網路連線
...
5. **檢查 OneDrive 是否已安裝/登入**：
   - Windows：確認 `C:\Users\[使用者]\OneDrive\` 路徑是否存在
   - macOS：確認 `~/Library/CloudStorage/OneDrive-*/` 路徑是否存在
   - 如果沒有安裝或未登入，告知使用者需要先完成安裝與登入
...
### 步驟二：安裝 OneDrive 桌面版（如果未安裝）

> 如果步驟零確認 OneDrive 已安裝並登入，跳過此步驟。

OneDrive 桌面版會在你的電腦上建立一個同步資料夾，Obsidian 的筆記存在這裡就能自動同步到雲端，換電腦也不會遺失。

> 🖐️ **需要手動操作**：請依照以下步驟操作。
>
> **下載與安裝：**
> 1. 開啟瀏覽器，前往 https://www.microsoft.com/onedrive/download
> 2. 下載並執行安裝檔
> 3. 安裝過程中按照指示完成（一路點「下一步」即可）
>
> **登入與設定：**
> 4. 安裝完成後會出現登入畫面，用你的 Microsoft 帳號登入
> 5. 系統匣（右下角）會出現 OneDrive 圖示
> 6. 點擊 OneDrive 圖示 → 齒輪 ⚙ → 「設定」
> 7. 確認同步資料夾位置：
>    - Windows：通常是 `C:\Users\[使用者]\OneDrive\`
>    - macOS：通常是 `~/Library/CloudStorage/OneDrive-你的信箱/`
> 8. 等待同步完成（OneDrive 圖示顯示 ✅ 打勾）
>
> 確認能看到 OneDrive 資料夾後，告訴 Claude「完成了」即可繼續。

---

### 步驟三：在 OneDrive 中建立 Vault 資料夾

請在使用者的 OneDrive 資料夾內建立 Obsidian vault 的目錄結構：

> 🖐️ **需要手動操作**：請詢問使用者想要的 vault 名稱（預設建議：`secondbrain`）

根據作業系統，在 OneDrive 資料夾內建立：
...
> 3. 選擇剛才在 OneDrive 建立的 vault 資料夾
...
>       "command": "mcpvault",
>       "args": [
>         "/Users/[使用者]/Library/CloudStorage/OneDrive-xxx/[vault名稱]"
>       ]
>     }
>   }
> }
...
> 例如：`"C:\\Users\\Evan\\OneDrive\\secondbrain"`
...
| OneDrive 同步衝突 | 避免在兩台裝置同時編輯同一篇筆記，等同步完成再操作 |
...
本懶人包預設使用 **OneDrive**（免費），如果你使用其他方案：

| 方案 | 差異 |
|------|------|
| **OneDrive**（本懶人包預設） | vault 建在 OneDrive 資料夾內即可 |
...
| 日期 | 版本 | 更新內容 |
|------|------|---------|
| 2026-05-20 | v0.1 | 建立 OneDrive 版本，取代 Google Drive 同步流程 |
